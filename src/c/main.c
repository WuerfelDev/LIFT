...
if( currentTime->tm_sec == 0 || bounds.origin.y == 0 ) {  // If minutes change
  ...
  if(calct>=layer_get_unobstructed_bounds(window_get_root_layer(window)).origin.y-PANELH){ //Benachrichtigung ist da
    bounds.origin.y = layer_get_unobstructed_bounds(window_get_root_layer(window)).origin.y-PANELH;
    printf("I: %i",layer_get_unobstructed_bounds(window_get_root_layer(window)).origin.y-PANELH);
  }else{
